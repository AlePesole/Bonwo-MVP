...
const schema = z.object({
  ...
  type: z.enum(["COMMUNITY", "OFFICIAL"]),
  ...
  activityIds: z.array(z.number()).min(1, "Select at least one activity"),
  trainingGoalIds: z.array(z.number()).min(1, "Select at least one training goal"),
});
...
function MuscleEntryRow({
  ...
  index: number;
  control: ReturnType<typeof useForm<PublicationForm>>["control"];
  setValue: ReturnType<typeof useForm<PublicationForm>>["setValue"];
  muscleGroups: MuscleGroupResponse[];
  onRemove: () => void;
}) {
...
function MusclesTabContent({
  ...
  fields: ReturnType<typeof useFieldArray<PublicationForm, "muscles">>["fields"];
  control: ReturnType<typeof useForm<PublicationForm>>["control"];
  setValue: ReturnType<typeof useForm<PublicationForm>>["setValue"];
  muscleGroups: MuscleGroupResponse[];
...
function useThumbnailUpload() {
  ...
  return { token, preview, uploading, error, reset, fileRef, handleFile };
}
...
function useVideoUpload() {
  ...
  return { token, preview, duration, uploading, error, reset, clear, fileRef, handleFile };
}
...
export function PublicationDialog({
  ...
  open: boolean;
  editing: ExercisePublicationResponse | null;
  onClose: () => void;
}) {
  ...
  useEffect(() => {
    if (open) {
      const exercise = editing?.exercise;
      thumb.reset(exercise?.thumbnail?.url ?? null);
      video.reset(exercise?.mainVideo ?? null);
      setServerError(null);
      reset({
        title: exercise?.title ?? "",
        level: exercise?.level ?? "INTERMEDIATE",
        type: editing?.type ?? "COMMUNITY",
        description: exercise?.description ?? "",
        instructions: exercise?.instructions ?? "",
        muscles: exercise?.muscles.map((m) => ({ subGroupId: m.subGroupId, activation: m.activation })) ?? [],
        equipmentIds: exercise?.equipment.map((e) => e.id) ?? [],
        activityIds: exercise?.activities.map((a) => a.id) ?? [],
        trainingGoalIds: exercise?.trainingGoals.map((g) => g.id) ?? [],
      });
    }
  }, [open, editing]);

  const mutation = useMutation({
    mutationFn: (data: PublicationForm) => {
      const muscles = data.muscles?.map((m) => ({
        subGroupId: Number(m.subGroupId),
        activation: m.activation,
      }));

      if (editing) {
        return publicationApi.update(editing.id, {
          title: data.title,
          level: data.level,
  ...
  const onSubmit = (data: PublicationForm) => {
    if (!editing) {
      if (!thumb.token || !video.token) {
        setServerError("Thumbnail and video are required to publish.");
        return;
      }
    }
    setServerError(null);
  ...
  const typeOptions = isAdmin
    ? PUBLICATION_TYPES
    : PUBLICATION_TYPES.filter((t) => t.value === "COMMUNITY");
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <Tabs defaultValue="info">
            ...
            <TabsContent value="info" className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  {editing ? (
                    <div className="flex items-center gap-3">
                      <div className="h-16 w-24 rounded-lg border border-border bg-muted flex items-center justify-center shrink-0 overflow-hidden relative">
                        {editing.exercise.mainVideo?.url ? (
                          <>
                            <video
                              src={editing.exercise.mainVideo.url}
                      ...
                      <p className="text-xs text-muted-foreground">Video cannot be changed after publish.</p>
                    </div>
                  ) : (
                    <div className="flex items-center gap-3">
                      <div
            ...
            <TabsContent value="details" className="mt-4">
              <DetailsTabContent
                ...
                onToggle={toggle}
              />
              {(errors.equipmentIds || errors.activityIds || errors.trainingGoalIds) && (
                <div className="mt-2 space-y-1">
                  {errors.equipmentIds && (
                    <p className="text-xs text-destructive">{errors.equipmentIds.message}</p>
                  )}
                  {errors.activityIds && (
                    <p className="text-xs text-destructive">{errors.activityIds.message}</p>
                  )}
                  {errors.trainingGoalIds && (
                    <p className="text-xs text-destructive">{errors.trainingGoalIds.message}</p>
                  )}
                </div>
              )}
            </TabsContent>
          ...
          <DialogFooter className="pt-2">
            ...
            <Button type="submit" disabled={mutation.isPending || thumb.uploading || video.uploading}>
              {mutation.isPending ? <Spinner size="sm" label="" /> : editing ? "Save changes" : "Publish"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
